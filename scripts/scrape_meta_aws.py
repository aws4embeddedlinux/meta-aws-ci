...
def generate_detail_page(branch, all_data, all_recipes, updated, version_pins=None):
    ...
    html = [
        ...
        "        .report-header { background: white; padding: 1.5rem;"
    ...
    if branch in DEPRECATED_BRANCHES:
        html.append(
            ...
            " No further updates will be made.</p>"
        )
    html.extend(
        [
            f"            <p><strong>Comparing:</strong> {branch} branch"
            f" vs {branch}-next branch from meta-aws git repository</p>",
            "            <p><strong>Legend:</strong>"
            " 🟢 Newest version | 🟡 Mid version"
            " | 🔴 Oldest version (comparing branch vs branch-next)</p>",
            "        </div>",
            "        <table>",
            "            <thead>",
            "                <tr>",
    ...
    for category in sorted(recipes_by_category.keys()):
        # Category header
        html.append('                <tr class="category-header">')
    ...
    html.extend(
        [
            ...
            "    <footer>",
            "        <p>Generated by "
            '<a href="https://github.com/aws4embeddedlinux/'
            'meta-aws-ci">meta-aws-ci</a></p>',
            "    </footer>",
            "</body>",
            "</html>",
        ]
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print("Cloning meta-aws repository...", file=sys.stderr)
        subprocess.run(
            [
                "git",
                "clone",
    ...
    for category in sorted(recipes_by_category.keys()):
        # Category header
        print('                <tr class="category-header">')
            ...
            for branch in branches:
                ...
                if isinstance(pin_note, dict) and pin_note.get("reason"):
                    ...
                    print(
                        ...
                        f'<a href="recipe-versions-{branch}.html'
                        f'#{recipe}" title="📌 {reason}">'
                        f"{status} 📌</a></td>"
                    )
                else:
                    print(
