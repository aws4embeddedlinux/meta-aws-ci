...
def main():
    ...
    print('    <style>')
    print('        .summary-cards { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; margin-bottom: 2rem; }')
    ...
    print('        .item-list li { padding: 0.5rem 0; border-bottom: 1px solid #f0f0f0; }')
    ...
    for data in all_data:
        ...
        print(f'                <span class="counts">{data["issue_count"]} issues, {data["pr_count"]} PRs</span>')
        print('            </div>')
        ...
        if data['issues']:
            ...
            for issue in sorted(data['issues'], key=lambda x: x['createdAt']):
                ...
                print(f'                <li>')
                print(f'                    <a href="{issue["url"]}" class="item-link">#{issue["number"]}: {issue["title"]}</a>')
        ...
        if data['prs']:
            ...
            for pr in sorted(data['prs'], key=lambda x: x['createdAt']):
                ...
                print(f'                <li>')
                print(f'                    <a href="{pr["url"]}" class="item-link">#{pr["number"]}: {pr["title"]}</a>')
                print(f'                    <span class="age">({age} days old)</span>')
                print(f'                </li>')
            print('            </ul>')
