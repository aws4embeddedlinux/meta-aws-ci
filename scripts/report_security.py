...
def main():
    ...
    print('    <style>')
    print('        .summary-cards { display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 1rem; margin-bottom: 2rem; }')
    print('        .summary-card { background: white; padding: 1rem; border-radius: 8px; text-align: center; border: 1px solid #dee2e6; }')
    ...
    print('        .repo-section { background: white; padding: 1.5rem; margin-bottom: 1rem; border-radius: 8px; }')
    print('        .repo-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem; }')
    print('        .repo-name { font-size: 1.25rem; font-weight: bold; color: #232f3e; }')
    print('        .severity-badge { display: inline-block; padding: 0.25rem 0.5rem; border-radius: 4px; font-size: 0.875rem; margin-right: 0.5rem; }')
    ...
    print('            <p><a href="index.html" class="back-link">← Back to Dashboard</a></p>')
    print(f'            <p><strong>Last Updated:</strong> {datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S UTC")}</p>')
    print('        </div>')
    ...
    for data in all_data:
        ...
        if data['total'] == 0:
            ...
            print('                <div>')
            if data['critical'] > 0:
                print(f'                    <span class="severity-badge critical">Critical: {data["critical"]}</span>')
            if data['high'] > 0:
                print(f'                    <span class="severity-badge high">High: {data["high"]}</span>')
            if data['medium'] > 0:
                print(f'                    <span class="severity-badge medium">Medium: {data["medium"]}</span>')
            if data['low'] > 0:
                print(f'                    <span class="severity-badge low">Low: {data["low"]}</span>')
            print('                </div>')
    ...
    print('    </main>')
    print('    <footer>')
    print('        <p>Generated by <a href="https://github.com/aws4embeddedlinux/meta-aws-ci">meta-aws-ci</a></p>')
    print('    </footer>')
    print('</body>')
    print('</html>')
